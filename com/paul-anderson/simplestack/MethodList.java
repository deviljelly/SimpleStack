	...
	public MethodList(Composite parent, int style) {
		...
		addMouseListener(new MouseListener() {
			...
			public void mouseDown(MouseEvent me) {
				...
				int method = Receiver.store.index[methodPosition-2];
...
//				SimpleStack.methodGraph.redraw();
		...
		addPaintListener(new PaintListener() {
			public void paintControl(PaintEvent e) {
				MethodList.this.paintControl(e);
			}
		});
	}
...
//		gc.setFont(font);
			...
			gc.drawString("All Methods:", PADDING, height, true);
			height+=16;
			height+=16;

			
			int[] index = Receiver.store.index;
			int pointer = 0;
			...
			while(method!=0) {
				...
				int hits = (int)store.methods[method+Store.METHOD_HITS];
