...
public class WaitingMethodList extends Canvas {
	...
	public WaitingMethodList(Composite parent, int style) {
		super(parent, style);
		...
		addMouseListener(new MouseListener() {
			...
			public void mouseDown(MouseEvent me) {
				...
				int method = Receiver.store.waitingIndex[methodPosition-2];
		...
		addPaintListener(new PaintListener() {
			public void paintControl(PaintEvent e) {
				WaitingMethodList.this.paintControl(e);
			}
		});
	}
...
//		gc.setFont(font);
			...
			int height = PADDING;
			gc.drawString("Waiting:", PADDING, height, true);
			height+=16;
			height+=16;

			int[] index = Receiver.store.waitingIndex;
			int pointer = 0;
			...
			float onePercent = ((float)Receiver.store.totalWaitingHits)/100.0f;

			while(method!=0) {

				int hits = (int)store.methods[method+Store.METHOD_WAITING_HITS];
